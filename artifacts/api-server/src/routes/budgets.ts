...
async function actualAmountFor(categoryId: number, month: string): Promise<number> {
...
async function toResponseRow(budget: typeof budgetsTable.$inferSelect) {
...
router.patch("/budgets/:id", async (req, res): Promise<void> => {
  ...
  if (parsed.data.plannedAmount !== undefined) update.plannedAmount = parsed.data.plannedAmount;
