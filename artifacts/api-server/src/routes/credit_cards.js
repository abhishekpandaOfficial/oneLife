...
import { ListCreditCardsResponse, CreateCreditCardBody, CreateCreditCardResponse, UpdateCreditCardParams, UpdateCreditCardBody, UpdateCreditCardResponse, DeleteCreditCardParams, } from "@workspace/api-zod";
...
router.patch("/credit-cards/:id", async (req, res) => {
    ...
    if (parsed.data.creditLimit !== undefined)
        update.creditLimit = parsed.data.creditLimit;
    if (parsed.data.outstandingAmount !== undefined)
        update.outstandingAmount = parsed.data.outstandingAmount;
    if (parsed.data.dueDate !== undefined)
        update.dueDate = toDateStr(parsed.data.dueDate);
    if (parsed.data.minimumDue !== undefined)
        update.minimumDue = parsed.data.minimumDue;
    const [card] = await db
