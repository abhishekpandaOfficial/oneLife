...
import { ListInvestmentsResponse, CreateInvestmentBody, CreateInvestmentResponse, UpdateInvestmentParams, UpdateInvestmentBody, UpdateInvestmentResponse, DeleteInvestmentParams, } from "@workspace/api-zod";
...
router.patch("/investments/:id", async (req, res) => {
    ...
    if (parsed.data.purchaseDate !== undefined)
        update.purchaseDate = toDateStr(parsed.data.purchaseDate);
    const [investment] = await db
