...
import { ListLoansResponse, CreateLoanBody, CreateLoanResponse, GetLoanParams, GetLoanResponse, UpdateLoanParams, UpdateLoanBody, UpdateLoanResponse, DeleteLoanParams, } from "@workspace/api-zod";
import { toDateStr } from "../lib/dates";
const router = Router();
async function monthsRemainingFor(loanId) {
...
async function withComputed(loan) {
...
router.patch("/loans/:id", async (req, res) => {
    ...
    if (parsed.data.loanType !== undefined)
        update.loanType = parsed.data.loanType;
    if (parsed.data.principalAmount !== undefined)
        update.principalAmount = parsed.data.principalAmount;
    if (parsed.data.outstandingAmount !== undefined)
        update.outstandingAmount = parsed.data.outstandingAmount;
    if (parsed.data.interestRate !== undefined)
        update.interestRate = parsed.data.interestRate;
    if (parsed.data.emiAmount !== undefined)
        update.emiAmount = parsed.data.emiAmount;
    if (parsed.data.tenureMonths !== undefined)
        update.tenureMonths = parsed.data.tenureMonths;
    if (parsed.data.startDate !== undefined)
        update.startDate = toDateStr(parsed.data.startDate);
    if (parsed.data.bankName !== undefined)
        update.bankName = parsed.data.bankName;
    if (parsed.data.bankLogoUrl !== undefined)
        update.bankLogoUrl = parsed.data.bankLogoUrl;
    if (parsed.data.disbursementDocUrl !== undefined)
        update.disbursementDocUrl = parsed.data.disbursementDocUrl;
    if (parsed.data.repaymentScheduleDocUrl !== undefined)
        update.repaymentScheduleDocUrl = parsed.data.repaymentScheduleDocUrl;
    if (parsed.data.penaltyRate !== undefined)
        update.penaltyRate = parsed.data.penaltyRate;
