...
import {
  ...
  insertSocialCircleSchema,
  insertSocialContentItemSchema,
  insertSocialFollowupSchema,
  insertSocialPersonSchema,
  insertSocialPublishQueueSchema,
  insertSocialAutomationRuleSchema,
  socialActivityTable,
  socialAiSuggestionsTable,
  socialAnalyticsTable,
  socialAutomationRulesTable,
  socialCampaignsTable,
  socialCirclesTable,
  socialConnectorsTable,
  socialContentItemsTable,
  socialFollowupsTable,
  socialHashtagsTable,
  socialPeopleTable,
  socialPlatformVersionsTable,
  socialPublishQueueTable,
} from "@workspace/db";
...
async function logActivity(eventType: string, message: string, actor = "OneSocial") {
...
async function generateVersions(contentId: number) {
  ...
  const tags = (content.tags || "OneLife,AI,Product").split(",").map((tag: string) => `#${tag.trim().replace(/^#/, "").replace(/\s+/g, "")}`).slice(0, 4).join(" ");
  ...
  const versions = [
    ...
    {
      contentId,
      platform: "linkedin_article" as const,
      title: content.title,
  ...
  return db.insert(socialPlatformVersionsTable).values(
    versions.map((version) => ({ ...version, characterCount: version.body.length })),
  ).returning();
}
...
async function summaryPayload() {
  ...
  const platformTotals = analytics.reduce<Record<string, number>>((acc, item) => {
...
router.get("/onesocial/capabilities", async (_req, res): Promise<void> => {
  res.json({
    module: "OneSocial",
    os: "ContentOS",
...
router.get("/onesocial/summary", async (_req, res): Promise<void> => {
  res.json(await summaryPayload());
});

router.post("/onesocial/campaigns", async (req, res): Promise<void> => {
...
router.post("/onesocial/content", async (req, res): Promise<void> => {
...
router.patch("/onesocial/content/:id", async (req, res): Promise<void> => {
...
router.delete("/onesocial/content/:id", async (req, res): Promise<void> => {
  const params = IdParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
...
router.post("/onesocial/content/:id/generate-platform-versions", async (req, res): Promise<void> => {
  const params = IdParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
...
router.post("/onesocial/schedule", async (req, res): Promise<void> => {
...
router.post("/onesocial/queue/:id/run", async (req, res): Promise<void> => {
  const params = IdParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
...
router.post("/onesocial/connectors/:id/check", async (req, res): Promise<void> => {
  const params = IdParams.safeParse(req.params);
...
router.post("/onesocial/connectors/:id/configure", async (req, res): Promise<void> => {
  const params = IdParams.safeParse(req.params);
  const body = z.object({
...
router.post("/onesocial/people", async (req, res): Promise<void> => {
...
router.post("/onesocial/circles", async (req, res): Promise<void> => {
...
router.post("/onesocial/followups", async (req, res): Promise<void> => {
...
router.post("/onesocial/automation-rules", async (req, res): Promise<void> => {
