...
import {
  ...
  WorkSalaryRecord,
} from "@workspace/api-zod/onework";
import { z } from "zod/v4";
import { toDateStr } from "../lib/dates";

const router: IRouter = Router();
let oneworkSchemaReady: Promise<void> | null = null;
...
function getMonthsList(startDateStr: string, endDateStr: string | null): string[] {
...
function generatedPfEntryForCompany(
  company: Awaited<ReturnType<typeof toCompanyRows>>[number],
  month: string,
  id: number,
) {
  return {
...
async function summaryPayload() {
  ...
  const generatedEntries: typeof dbEntries = [];
...
function moneyFromMatches(text: string, patterns: RegExp[]) {
...
function parseSalaryAmounts(rawText: string, fileName = "") {
...
function financialYearsForCompany(startDate: string, endDate: string | null) {
  const start = new Date(startDate);
  const end = endDate ? new Date(endDate) : new Date();
  const years: string[] = [];
...
function essentialFolderDefinitions(company: { startDate: string; endDate: string | null }) {
  return [
...
function folderNameForDocumentType(type: string, documentDate?: string | null) {
...
async function ensureEssentialFolders(companyId: number) {
  await ensureOneworkSchema();
...
router.get("/onework/capabilities", async (_req, res): Promise<void> => {
  res.json({
    module: "OneWork",
    os: "CareerOS",
...
router.post("/onework/companies/:id/essential-folders", async (req, res): Promise<void> => {
  const params = UpdateWorkCompanyParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
...
router.post("/onework/salary-documents", async (req, res): Promise<void> => {
  await ensureOneworkSchema();
...
router.post("/onework/google-drive/connect", async (req, res): Promise<void> => {
  await ensureOneworkSchema();
...
router.post("/onework/google-drive/disconnect", async (_req, res): Promise<void> => {
  await ensureOneworkSchema();
  const [existing] = await db.select().from(oneworkProfileTable);
  if (existing) {
...
router.post("/onework/google-drive/sync", async (_req, res): Promise<void> => {
  await ensureOneworkSchema();
...
router.post("/onework/google-drive/files/sync-single", async (req, res): Promise<void> => {
  await ensureOneworkSchema();
  const { id } = req.body;
  if (!id) {
...
router.get("/onework/google-drive/files", async (_req, res): Promise<void> => {
  await ensureOneworkSchema();
  const docs = await db.select().from(workDocumentsTable);
...
router.delete("/onework/google-drive/files/:id", async (req, res): Promise<void> => {
  await ensureOneworkSchema();
  const { id } = req.params;
  if (!id) {
...
router.post("/onework/epfo/sync", async (req, res): Promise<void> => {
  await ensureOneworkSchema();
...
router.post("/onework/epfo/parse-passbook", async (req, res): Promise<void> => {
  ...
  if (passbookText.toLowerCase().includes("solera") || passbookText.toLowerCase().includes("wells fargo") || passbookText.toLowerCase().includes("abhishek")) {
    companiesFound.push(
      { name: "Solera (Smart Drive Systems Pvt. Ltd.)", records: 31, amount: 930000 },
      { name: "Accion Labs", records: 7, amount: 168000 },
      { name: "Wells Fargo", records: 27, amount: 540000 }
    );
  } else {
    companiesFound.push(
      { name: "EPFO Member Establishment A", records: 12, amount: 72000 }
    );
  }
...
router.post("/onework/cv/parse", async (req, res): Promise<void> => {
