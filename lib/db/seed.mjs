...
async function main() {
  ...
  try {
    ...
    const categories = [
      ...
      ["Interest Income", "income", "#14b8a6", "BadgeDollarSign"],
      ["Dividends", "income", "#6366f1", "TrendingUp"],
      ["Rental Income", "income", "#16a34a", "Home"],
      ["Business Income", "income", "#0891b2", "Store"],
      ["Bonus", "income", "#84cc16", "Sparkles"],
      ["Rent", "expense", "#ef4444", "Home"],
      ["Society Maintenance", "expense", "#f97316", "Building2"],
      ["Home Maintenance", "expense", "#f97316", "Hammer"],
      ["Groceries", "expense", "#f59e0b", "ShoppingCart"],
      ["Milk & Dairy", "expense", "#fde047", "Milk"],
      ["Vegetables & Fruits", "expense", "#22c55e", "Apple"],
      ["Dining Out", "expense", "#f97316", "UtensilsCrossed"],
      ["Tea & Snacks", "expense", "#fb923c", "Coffee"],
      ["Utilities", "expense", "#eab308", "Zap"],
      ["Electricity Bill", "expense", "#facc15", "Lightbulb"],
      ["LPG Gas", "expense", "#ea580c", "Flame"],
      ["Internet & Mobile", "expense", "#06b6d4", "Wifi"],
      ["Transport", "expense", "#8b5cf6", "Car"],
      ["Metro & Local Train", "expense", "#4f46e5", "Train"],
      ["Auto & Cab", "expense", "#7c3aed", "CarTaxiFront"],
      ["Fuel", "expense", "#a855f7", "Fuel"],
      ["Parking & Tolls", "expense", "#7c3aed", "ParkingCircle"],
      ["Entertainment", "expense", "#ec4899", "Film"],
      ["Subscriptions", "expense", "#db2777", "Repeat"],
      ["Healthcare", "expense", "#14b8a6", "HeartPulse"],
      ["Medicines", "expense", "#0d9488", "Pill"],
      ["Fitness", "expense", "#22c55e", "Dumbbell"],
      ["Shopping", "expense", "#6366f1", "ShoppingBag"],
      ["Clothing & Footwear", "expense", "#4f46e5", "Shirt"],
      ["Education", "expense", "#2563eb", "GraduationCap"],
      ["School Fees", "expense", "#1d4ed8", "School"],
      ["Books & Stationery", "expense", "#3b82f6", "BookOpen"],
      ["Childcare", "expense", "#f43f5e", "Baby"],
      ["Travel", "expense", "#0ea5e9", "Plane"],
      ["Train Tickets", "expense", "#0284c7", "Ticket"],
      ["Flights & Hotels", "expense", "#0369a1", "Hotel"],
      ["Insurance Premiums", "expense", "#0284c7", "ShieldCheck"],
      ["Loan EMI", "expense", "#dc2626", "Landmark"],
      ["Credit Card Payments", "expense", "#9333ea", "CreditCard"],
      ["UPI Transfers", "expense", "#64748b", "QrCode"],
      ["Taxes", "expense", "#475569", "ReceiptText"],
      ["Income Tax & TDS", "expense", "#334155", "FileText"],
      ["Gifts & Donations", "expense", "#e11d48", "Gift"],
      ["Festivals & Puja", "expense", "#f43f5e", "Lamp"],
      ["Personal Care", "expense", "#d946ef", "Sparkles"],
      ["Pets", "expense", "#84cc16", "PawPrint"],
      ["Bank Fees", "expense", "#64748b", "CircleDollarSign"],
      ["Domestic Help", "expense", "#9333ea", "HandHeart"],
      ["Miscellaneous", "expense", "#94a3b8", "MoreHorizontal"],
    ];
    const categoryIds = {};
    for (const [name, type, color, icon] of categories) {
      const { rows: existingCategory } = await client.query(
        "SELECT id FROM categories WHERE lower(name) = lower($1) AND type = $2 ORDER BY id LIMIT 1",
        [name, type],
      );

      if (existingCategory.length > 0) {
        const { rows } = await client.query(
          "UPDATE categories SET color = $1, icon = $2 WHERE id = $3 RETURNING id",
          [color, icon, existingCategory[0].id],
        );
    ...
    const { rows: existingTransactions } = await client.query("SELECT COUNT(*)::int AS count FROM transactions");
    if (existingTransactions[0].count > 0) {
      await client.query("COMMIT");
      console.log("Categories synced. Sample transaction data already present, skipping.");
      return;
    }
